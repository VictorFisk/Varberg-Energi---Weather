...
# Wind urban correction factor
# Getterön is a coastal station — winds are typically higher than in the city.
# A factor of 0.85 adjusts coastal wind readings to better reflect urban conditions.
# Range: 0.7 (strong dampening) to 1.0 (no adjustment). Default: 0.85
WIND_URBAN_FACTOR = 0.85

# Station metadata (exported to JSON for display in the HTML report)
STATION_NAME = "Getterön"
STATION_TYPE = "Kuststation (SMHI CORE-nät)"
STATION_NOTE = "Vindmätningar justerade med faktor {factor:.2f} för stadsmiljö"  # formatted at export
...
def init_db(db_path: Path) -> sqlite3.Connection:
    ...
    # Migration: add source column if it doesn't exist yet (for existing databases)
    try:
        cur.execute("ALTER TABLE daily ADD COLUMN source TEXT DEFAULT 'live'")
        con.commit()
        log.info("  DB migration: added 'source' column to daily table")
...
def compute_heat_index(temp_mean: float, wind_mean: float, solar_mean: float,
                       wind_factor: float = 1.0) -> tuple[float, float, float]:
    """
    ...
    wind_addition = min(12.0, (wind_mean * wind_factor) * 1.2)
...
def aggregate_daily(con: sqlite3.Connection, dates: list[str] | None = None) -> int:
    ...
    for d in dates:
        ...
        wind_add, sol_red, heat_idx = compute_heat_index(temp_mean, wind_mean, solar_mean, WIND_URBAN_FACTOR)
        ...
        cur.execute("""
            ...
            ON CONFLICT(date) DO UPDATE SET
                ...
                source=CASE WHEN daily.source='backfill' THEN 'live' ELSE daily.source END
        """, (
...
def export_json(con: sqlite3.Connection, export_path: Path):
    ...
    # Daily data — export all rows for the archive page
    cur.execute("""
        SELECT * FROM daily
        ORDER BY date ASC
    """)
    ...
    payload = {
        ...
        "station_lat": LAT,
        "station_lon": LON,
        "station_note": STATION_NOTE.format(factor=WIND_URBAN_FACTOR),
        "wind_urban_factor": WIND_URBAN_FACTOR,
        "lat": LAT,
        "lon": LON,
    ...
    # Export daily_archive.json — full daily history for the data explorer
    # Kept separate so it can grow large without bloating weather_data.json
    archive_path = export_path.parent / "daily_archive.json"
    archive_payload = {
        "generated_at": datetime.now(timezone.utc).isoformat(),
        "daily": history_daily,
    }
    with open(archive_path, "w", encoding="utf-8") as f:
        json.dump(archive_payload, f, ensure_ascii=False, indent=2, default=str)
    log.info(f"  ✓ Exported daily archive → {archive_path} ({len(history_daily)} days)")
